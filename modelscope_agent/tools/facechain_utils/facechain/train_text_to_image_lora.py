...
sys.path.append('../../')
from modelscope_agent.tools.facechain_utils.facechain.inference import data_process_fn
from modelscope_agent.tools.facechain_utils.facechain.utils import snapshot_download
...
def get_rot(image):
    ...
    mean = np.array([103.53, 116.28, 123.675], dtype=np.float32).reshape(
        (1, 1, 3))
    norm = np.array([0.01742919, 0.017507, 0.01712475],
...
def prepare_dataset(instance_images: list, output_dataset_dir):
    print('####_______if_____文件夹', output_dataset_dir)
    if not os.path.exists(output_dataset_dir):
        os.makedirs(output_dataset_dir)

        print('####_______文件夹', output_dataset_dir)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=('The dataset dir after processing'),
    )
    parser.add_argument(
        '--image_column',
        type=str,
    ...
    parser.add_argument(
        '--use_peft',
        action='store_true',
        help='Whether to use peft to support lora')
    parser.add_argument(
        '--lora_r',
        type=int,
        default=4,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help='Lora alpha, only used if lora is True')
    parser.add_argument(
        '--lora_dropout',
        type=float,
        default=0.0,
    ...
    parser.add_argument(
        ...
        default=4,
        help=
        'Lora rank for text encoder, only used if `use_lora` and `train_text_encoder` are True',
    )
    parser.add_argument(
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        args.dataset_name = [
            os.path.join(args.dataset_name, x)
            for x in os.listdir(args.dataset_name)
        ]
        shutil.rmtree(args.output_dataset_name, ignore_errors=True)
        prepare_dataset(args.dataset_name, args.output_dataset_name)
        ## Our data process fn
        data_process_fn(
            input_img_dir=args.output_dataset_name, use_data_process=True)
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint == 'fromfacecommon':
            weight_model_dir = snapshot_download(
    ...
    if accelerator.is_main_process:
        if args.use_peft:
            ...
            state_dict = get_peft_model_state_dict(
                unwarpped_unet, state_dict=accelerator.get_state_dict(unet))
            lora_config[
                'peft_config'] = unwarpped_unet.get_peft_config_as_dict(
                    inference=True)
            if args.train_text_encoder:
                unwarpped_text_encoder = accelerator.unwrap_model(text_encoder)
                text_encoder_state_dict = get_peft_model_state_dict(
                    unwarpped_text_encoder,
            ...
            accelerator.save(
                state_dict,
                os.path.join(args.output_dir, f'{global_step}_lora.pt'))
            with open(
    ...
    if args.use_peft:
        ...
        def load_and_set_lora_ckpt(pipe, ckpt_dir, global_step, device, dtype):
            ...
            unet_lora_ds = {
                ...
                if 'text_encoder_' not in k
            }
            text_encoder_lora_ds = {
                k.replace('text_encoder_', ''): v
                for k, v in lora_checkpoint_sd.items() if 'text_encoder_' in k
            }
            ...
            if 'text_encoder_peft_config' in lora_config:
                ...
                pipe.text_encoder = LoraModel(text_encoder_config,
                                              pipe.text_encoder)
        ...
        pipeline = load_and_set_lora_ckpt(pipeline, args.output_dir,
                                          global_step, accelerator.device,
                                          weight_dtype)
