        ...
        for i in range(len(labels)):
            if scores[i] > 0.8:
                if labels[i] == 'Torso-skin':
                    ...
                    mask_hair += masks[i]
                elif labels[i] == 'UpperClothes' or labels[
                        i] == 'Coat' or labels[i] == 'Dress' or labels[
                            i] == 'Pants' or labels[i] == 'Skirt':
                    mask_cloth += masks[i]
...
def main_diffusion_inference_tryon(inpaint_image,
                                   strength,
                                   output_img_size,
                                   pos_prompt,
                                   neg_prompt,
    ...
    model_dir0 = snapshot_download(
        'damo/face_chain_control_model', revision='v1.0.2')
    ...
    openpose_image, handbox = dwprocessor(
    ...
    controlnet = [
        ...
        ControlNetModel.from_pretrained(
            os.path.join(model_dir,
                         'model_controlnet/control_v11p_sd15_depth'),
            torch_dtype=dtype),
        ControlNetModel.from_pretrained(
    ...
    mask1 = segment(
        segmentation_pipeline, inpaint_im, ksize=5, return_human=True)

    canny_image = cv2.Canny(np.array(inpaint_im, np.uint8), 80, 200)[:, :,
                                                                     None]
    ...
    for hbox in handbox:
        depth_input = Image.fromarray(
            np.array(inpaint_im)[hbox[1]:hbox[3], hbox[0]:hbox[2]])
    ...
    for i in range(1):
        ...
        cloth_mask_warp = segment(
            segmentation_pipeline, inpaint_im, return_cloth=True, ksize=5)
        cloth_mask = segment(
            segmentation_pipeline, inpaint_im, return_cloth=True, ksize=15)
        inpaint_with_mask = (cloth_mask_warp[:, :, None]
    ...
    for i in range(1):
        ...
        image_human = (
            np.array(images_human[i]) *
            (1.0 - soft_cloth_mask_warp[:, :, None])
            + np.array(inpaint_im) * soft_cloth_mask_warp[:, :, None]).astype(
                np.uint8)
...
def main_model_inference(inpaint_image,
    ...
    if use_main_model:
        ...
        return main_diffusion_inference_tryon(
