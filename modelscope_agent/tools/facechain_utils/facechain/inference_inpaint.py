...
from .data_process.preprocessing import Blipv2
...
def data_process_fn(input_img_dir, use_data_process):
    ...
    if use_data_process:
        ...
        _process = multiprocessing.Process(
            target=_data_process_fn_process, args=(input_img_dir, ))
        _process.start()
        _process.join()
...
def crop_and_paste(Source_image,
    ...
    if use_warp:
        ...
        output = mask * np.float32(Target_image) + (
            1 - mask) * np.float32(warped)
    else:
        mask = np.float32(np.array(Source_image_mask) == 0)
        output = mask * np.float32(Target_image) + (
            1 - mask) * np.float32(Source_image)
    return output, mask
...
def main_diffusion_inference_inpaint(inpaint_image,
                                     strength,
                                     output_img_size,
                                     pos_prompt,
                                     neg_prompt,
    ...
    det_pipeline = pipeline(Tasks.face_detection,
    ...
    trigger_styles = [
        'a boy, children, ', 'a girl, children, ', 'a handsome man, ',
        'a beautiful woman, ', 'a mature man, ', 'a mature woman, '
    ]
    ...
    openpose = OpenposeDetector.from_pretrained(
        os.path.join(model_dir, 'model_controlnet/ControlNet'))
    controlnet = ControlNetModel.from_pretrained(
        os.path.join(model_dir, 'model_controlnet/control_v11p_sd15_openpose'),
        torch_dtype=dtype)
    ...
    pipe = StableDiffusionControlNetPipeline.from_pretrained(
        base_model_path, controlnet=controlnet, torch_dtype=dtype)
    lora_style_path = style_model_path
    lora_human_path = lora_model_path
    pipe = merge_lora(
        pipe, lora_style_path, multiplier_style, from_safetensor=True)
    pipe = merge_lora(
        pipe, lora_human_path, multiplier_human, from_safetensor=False)
    pipe = pipe.to('cuda')
    image_faces = []
    for i in range(1):
        image_face = pipe(
            prompt=trigger_style + add_prompt_style + pos_prompt,
    ...
    pipe = merge_lora(
        pipe, style_model_path, multiplier_style, from_safetensor=True)
    pipe = merge_lora(
        pipe, lora_model_path, multiplier_human, from_safetensor=False)
    pipe = pipe.to('cuda')
    ...
    for i in range(len(images_human)):
        ...
        canny_image = Image.fromarray(
            np.concatenate([canny_image, canny_image, canny_image], axis=2))
        openpose_image = openpose(
            np.array(im, np.uint8), include_hand=True, include_face=True)
        read_control = [openpose_image, canny_image]
        inpaint_mask, human_mask = segment(
            segmentation_pipeline,
            images_human[i],
...
def main_diffusion_inference_inpaint_multi(inpaint_images,
                                           strength,
                                           output_img_size,
                                           pos_prompt,
                                           neg_prompt,
    ...
    for i in range(1):
        inpaint_im = inpaint_images[i]
        inpaint_bbox, inpaint_keypoints = call_face_crop(
            det_pipeline, inpaint_im, 1.1)
        eye_height = int(
            (inpaint_keypoints[0, 1] + inpaint_keypoints[1, 1]) / 2)
        canny_image = cv2.Canny(np.array(inpaint_im, np.uint8), 100, 200)[:, :,
                                                                          None]
        mask = segment(
            segmentation_pipeline, inpaint_im, ksize=0.05, eyeh=eye_height)
...
def main_model_inference(inpaint_image,
                         strength,
                         output_img_size,
                         pos_prompt,
                         neg_prompt,
...
def main_model_inference_multi(inpaint_image,
    ...
    if use_main_model:
        ...
        return main_diffusion_inference_inpaint_multi(
            inpaint_image, strength, output_img_size, pos_prompt, neg_prompt,
            input_img_dir, base_model_path, style_model_path, lora_model_path,
            **multiplier_style_kwargs, **multiplier_human_kwargs)
...
class GenPortrait_inpaint:
    ...
    def __call__(self,
                 ...
                 input_img_dir2=None,
                 base_model_path=None,
                 lora_model_path1=None,
                 lora_model_path2=None,
                 sub_path=None,
                 revision=None):
        ...
        if lora_model_path1 != None:
            ...
            gen_results = main_model_inference(
                ...
                self.use_main_model,
                input_img_dir=input_img_dir1,
                lora_model_path=lora_model_path1,
            ...
            final_gen_results_new = [
                inpaint_img_le, inpaint_img_le, inpaint_img_le
            ]

        for i in range(1):
            cv2.imwrite('tmp_inpaint_left_{}.png'.format(i),
                        final_gen_results_new[i])
        ...
        if lora_model_path2 != None and self.num_faces == 2:
            ...
            gen_results = main_model_inference_multi(
                inpaint_imgs,
                self.strength,
                512,
                self.pos_prompt,
                ...
                self.multiplier_human,
                self.use_main_model,
                input_img_dir=input_img_dir2,
                lora_model_path=lora_model_path2,
                base_model_path=base_model_path)
            ...
            for i in range(len(final_gen_results)):
                ...
                final_inpaint_rst = (
                    inpaint_img_rst.astype(np.float32)
                    * mask_large.astype(np.float32)
                    + final_gen_results_new[i].astype(np.float32) *
                    (1.0 - mask_large.astype(np.float32))).astype(np.uint8)
                print('Finish masking.')
