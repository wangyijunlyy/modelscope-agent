...
class DeepDanbooruModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.n_Conv_1 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=64, out_channels=256)
        self.n_Conv_2 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=64, out_channels=64)
        self.n_Conv_3 = nn.Conv2d(
            kernel_size=(3, 3), in_channels=64, out_channels=64)
        self.n_Conv_4 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=64, out_channels=256)
        self.n_Conv_5 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=256, out_channels=64)
        self.n_Conv_6 = nn.Conv2d(
            kernel_size=(3, 3), in_channels=64, out_channels=64)
        self.n_Conv_7 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=64, out_channels=256)
        self.n_Conv_8 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=256, out_channels=64)
        self.n_Conv_9 = nn.Conv2d(
            kernel_size=(3, 3), in_channels=64, out_channels=64)
        self.n_Conv_10 = nn.Conv2d(
            kernel_size=(1, 1), in_channels=64, out_channels=256)
    ...
    def load_state_dict(self, state_dict, **kwargs):
        ...
        super(DeepDanbooruModel, self).load_state_dict(
            {k: v
             for k, v in state_dict.items() if k != 'tags'})
...
def resize_image(im, width, height):
    ...
    if ratio < src_ratio:
        ...
        res.paste(
            ...
            box=(0, 0))
        res.paste(
            resized.resize((width, fill_height),
        ...
        res.paste(
            ...
            box=(0, 0))
        res.paste(
            resized.resize((fill_width, height),
...
class DeepDanbooru:
    ...
    def __init__(self):
        ...
        pretrain_model_path = os.path.join(snapshot_path,
                                           'model-resnet_custom_v3.pt')

        self.model.load_state_dict(
            torch.load(pretrain_model_path, map_location='cpu'))
        self.model.eval()
        ...
        if alpha_sort:
            ...
            tags = [
                tag for tag, _ in sorted(
                    probability_dict.items(), key=lambda x: -x[1])
            ]
