...
def pad_to_square(im):
    h, w, _ = im.shape
    ns = int(max(h, w) * 1.5)
    im = cv2.copyMakeBorder(im, int((ns - h) / 2), (ns - h) - int(
        (ns - h) / 2), int((ns - w) / 2), (ns - w) - int((ns - w) / 2),
...
def rotate(im, keypoints):
    ...
    dst_mean_face = np.asarray([
        0.31074522411511746, 0.2798131190011913, 0.6892073313037804,
        0.2797830232679366, 0.49997367716346774, 0.5099309118810921,
        0.35811903020866753, 0.7233174007629063, 0.6418878095835022,
        0.7232890570786875
    ])
    ...
    pts1 = np.float64(
        np.matrix([[point[0], point[1]] for point in points_array]))
    pts2 = np.float64(
        np.matrix([[point[0], point[1]] for point in dst_mean_face]))
    ...
    im = pad_to_square(im)
    ns = int(1.5 * max(h, w))
    M = cv2.getRotationMatrix2D((ns / 2, ns / 2),
...
def get_mask_head(result):
    ...
    _, mask_head = cv2.threshold((mask_head * 255).astype(np.uint8), 127, 255,
                                 cv2.THRESH_BINARY)
    contours, hierarchy = cv2.findContours(mask_head, cv2.RETR_TREE,
...
class Blipv2():
    ...
    def __init__(self):
        ...
        self.segmentation_pipeline = pipeline(
            ...
            model_revision='v1.0.1')
        self.fair_face_attribute_func = pipeline(
    ...
    def __call__(self, imdir):
        ...
        for imname in imlist:
            try:
                ...
                if len(bboxes) > 1:
                    ...
                    print('Detecting no face, do not use image {}.'.format(
                        imname))
                    continue
                else:
                ...
                if len(bboxes) > 1:
                    ...
                    if areas_new[0] < 4 * areas_new[1]:
                        print(
                            'Detecting multiple faces after rotation, do not use image {}.'
                            .format(imname))
                        continue
                    else:
                        bbox = bboxes[idxs[0]]
                    ...
                    print(
                        'Detecting no face after rotation, do not use this image {}'
                        .format(imname))
                    continue
                else:
                    bbox = bboxes[0]
        ...
        for i in range(len(result_list)):
            ...
            info_dict = {
                'file_name': imgs_list[i],
                'text': '<fcsks>, ' + generated_text
            }
