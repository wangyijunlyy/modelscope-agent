...
def main_diffusion_inference(pos_prompt,
                             neg_prompt,
                             input_img_dir,
                             base_model_path,
                             style_model_path,
    ...
    pipe = StableDiffusionPipeline.from_pretrained(
        base_model_path, safety_checker=None, torch_dtype=torch.float32)
    ...
    pipe = merge_lora(
        ...
        lora_human_path,
        multiplier_human,
        from_safetensor=lora_human_path.endswith('safetensors'))
    print(
        f'multiplier_style:{multiplier_style}, multiplier_human:{multiplier_human}'
    )
    ...
    for tag in tags_all:
        if tags_all.count(tag) > 0.5 * cnt:
            if ('hair' in tag or 'face' in tag or 'mouth' in tag
                    or 'skin' in tag or 'smile' in tag):
...
def main_diffusion_inference_pose(pose_model_path,
                                  pose_image,
                                  pos_prompt,
                                  neg_prompt,
                                  input_img_dir,
    ...
    controlnet = ControlNetModel.from_pretrained(
        pose_model_path, torch_dtype=torch.float32)
...
def main_diffusion_inference_multi(pose_model_path,
                                   pose_image,
                                   pos_prompt,
                                   neg_prompt,
                                   input_img_dir,
...
def main_model_inference(pose_model_path,
                         pose_image,
                         use_depth_control,
                         pos_prompt,
                         neg_prompt,
    ...
    if use_main_model:
        ...
        if pose_image is None:
            return main_diffusion_inference(pos_prompt, neg_prompt,
            ...
            if use_depth_control:
                return main_diffusion_inference_multi(
                    pose_model_path, pose_image, pos_prompt, neg_prompt,
                    input_img_dir, base_model_path, style_model_path,
                    lora_model_path, **multiplier_style_kwargs,
                    **multiplier_human_kwargs)
            else:
