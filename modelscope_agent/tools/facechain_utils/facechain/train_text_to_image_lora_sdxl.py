...
from .inference import data_process_fn
...
def parse_args():
    ...
    parser.add_argument(
        '--use_swift',
        action='store_true',
        help='Whether to use swift to support lora')
    parser.add_argument(
        '--lora_r',
        type=int,
        default=4,
...
def main():
    ...
    def encode_prompt(text_encoders,
                      tokenizers,
                      prompt,
    ...
    if accelerator.is_main_process:
        if args.use_peft:
            ...
            unwarpped_unet.save_pretrained(
                os.path.join(args.output_dir, 'swift'))
            if args.train_text_encoder:
                unwarpped_text_encoder_one = accelerator.unwrap_model(
                    text_encoder_one)
                unwarpped_text_encoder.save_pretrained(
                    os.path.join(args.output_dir, 'text_encoder_one'))
                unwarpped_text_encoder_two = accelerator.unwrap_model(
                    text_encoder_two)
                unwarpped_text_encoder.save_pretrained(
                    os.path.join(args.output_dir, 'text_encoder_two'))
        else:
