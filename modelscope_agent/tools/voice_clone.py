...
def auto_label(wav_path, user_id):
    ...
    shutil.rmtree(wav_directory,ignore_errors=True)
...
def infer(model_dir, text, user_id):
    ...
    shutil.rmtree('wav_utils',ignore_errors=True)
    os.makedirs('wav_utils', exist_ok=True)
    output_wav_path = f'wav_utils/output_audio_{user_id}.wav'
...
class VoiceCloneTool(BaseTool):
    description = '根据用户输入的参考音频，克隆生成一段音频.'
    name = 'voice_gen'
    parameters: list = [{
        'name': 'audio_path',
        'type': 'string',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        print('---------------------\nstart infer')
        output_wav_path = infer(pretrain_work_dir, text='你好，我是你创建的数字分身', user_id=user_id)
        return AudioWrapper(output_wav_path),f'用户id是{user_id}'
                

@register_tool('voice_infer')
class VoiceInferTool(BaseTool):
    description = '根据用户的文本内容，推理生成一段新的音频。'
    name = 'voice_infer'
    parameters: list = [{
        'name': 'user_id',
        'type': 'string',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        if params['text'] is None:
            raise ValueError('没有获取到音频文本内容')
        pretrain_work_dir = os.path.join(WORK_DIR, f"pretrain_{params['user_id']}")
        print('---------------------\nstart infer')
        output_wav_path = infer(pretrain_work_dir, text=params['text'], user_id=params['user_id'])
        return AudioWrapper(output_wav_path)
